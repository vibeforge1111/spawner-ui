...
- `/api/spark/run` must not generate a start after a terminal callback. Board, Trace, and Telegram consumers must independently preserve terminal precedence when handling delayed or replayed events.
